...
head $x -n -1 > p.txt
tail $x -n 1 > t.txt
cat p.txt z.txt t.txt > protein_processed.gro
value1=`cat $x | wc -l` 
deletingvalue1=3
...
sed -i "2 i\ $valuefinal" protein_processed.gro 
sed -i '3d' protein_processed.gro
rm p.txt z.txt t.txt
echo "-----protein_processed.gro is ready-----"
}
echo "enter conf.gro"
read first
echo "enter lig.gro"
